...
def calculate_statistics(date_from, date_to, anonymous=None, group_badge=None, 
    ...
    planning_cells_cache = load_planning_cells_cache()

    # === Загружаем volunteers ===
    volunteers = load_volunteers(stat_date_from, stat_date_to, anonymous, group_badge)
    group_badge_volunteers = []
    if group_badge is False:
        group_badge_volunteers = load_volunteers(stat_date_from, stat_date_to, anonymous, True)
    ...
    if group_badge is False:
        add_group_badge_excess_to_regular_plan(
            store,
            stat_date_from,
            stat_date_to,
            group_badge_volunteers,
            planning_cells_cache,
            load_group_badge_kitchens()
        )
...
def add_group_badge_excess_to_regular_plan(
        store, date_from, date_to, volunteers, planning_cells_cache, group_badge_kitchens):
    for current_day in arrow.Arrow.range('day', date_from, date_to):
        ...
        for vol in volunteers:
            ...
            for meal_time in get_meal_times_for_day(vol, current_day):
                active_counts[(vol['group_badge_id'], meal_time, vol['is_vegan'])] += 1

        for (badge_id, meal_time, is_vegan), active_count in active_counts.items():
            cell = planning_cells_cache.get((badge_id, meal_time, current_date_str))
            if not cell:
                continue

            planned_amount = cell.get('amount_vegan') if is_vegan else cell.get('amount_meat')
            excess_amount = active_count - (planned_amount or 0)
            if excess_amount <= 0:
                continue

            kitchen_id = group_badge_kitchens.get(badge_id)
            if not kitchen_id:
                kitchen_id = 1
            ...
            for is_vegan in [False, True]:
                ...
                if apply_predict_alg_to_group_badge:
                    ...
                    if prev_day:
                        ...
                        predict_amount = predict_raw_amount if prev_predict_raw == 0 or prev_fact == 0 else predict_raw_amount * (prev_fact / prev_predict_raw)
