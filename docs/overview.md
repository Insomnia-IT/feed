...
- Боковое меню (`components/sider/sider.tsx`, `sider.module.css`): высота панели
  ...
  прокручивается; «Последняя синхронизация» (`position: absolute; bottom: 48px`) и
  кнопка сворачивания Ant Design (`position: fixed; bottom: 0`) закреплены у нижнего
  края экрана. На десктопе кнопка сворачивания — внизу панели (chevron ←/→);
