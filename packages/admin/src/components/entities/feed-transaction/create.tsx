...
export const FeedTransactionCreate: FC<IResourceComponentsProps> = () => {
    ...
    const { selectProps: volSelectProps } = useSelect<VolEntity>({ resource: 'volunteers', optionLabel: 'name' });
    const { selectProps: kitchenSelectProps } = useSelect<KitchenEntity>({ resource: 'kitchens', optionLabel: 'name' });
    ...
    const onTimeChange = useCallback(
        (value) => form.setFieldValue('ulid', value ? ulid(value.unix()) : undefined),
        [form]
    );
